...
(add-to-list 'org-file-apps
             (cons (concat org-player-file-extensions-regexp
                           "::[0-9]+:[0-9]+\\(:[0-9]+\\|\\)")
                   (lambda (file link) (org-player-play-file file search))))
...
(defun transcribe-from-dired ()
  "Uses the cygstart command to open the file at point."
  (interactive)
  (shell-command
   (shell-quote-argument (concat "python "
                                (substitute-in-file-name "$HOME/bin/aws-transcribe.py")
                                " '"
